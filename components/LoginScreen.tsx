...
export const LoginScreen: React.FC<LoginScreenProps> = ({ hasValidConfig, onSaveConfig }) => {
  ...
  // robustly get the origin without path or trailing slash
      ...
      <div className="bg-slate-900 p-8 rounded-2xl shadow-2xl border border-slate-800 max-w-md w-full text-center">
        ...
        {!hasValidConfig ? (
          ...
          <>
            ...
            <button 
              onClick={() => onSaveConfig('')} // Clearing it allows re-entry
        ...
        <div className="mt-8 pt-6 border-t border-slate-800">
           ...
           {showHelp && (
             <div className="text-left mt-4 bg-slate-950 p-4 rounded-lg border border-slate-800 text-xs text-slate-400">
                <div className="mb-3">
                  <p className="font-bold text-slate-200 mb-1">Getting "Error 400: invalid_request"?</p>
                  <p className="mb-2">You must add this specific URL to your <strong>Authorized JavaScript Origins</strong> in the Google Cloud Console.</p>
                  ...
                  <p className="mt-2 text-amber-500 italic">⚠️ Do NOT copy from your browser's address bar. It contains extra paths that Google rejects.</p>
                </div>
